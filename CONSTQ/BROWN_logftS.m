...
function  [cqtrans, tt, freqs, kerncos, kernsin] = BROWN_logftS(infile, varargin);
...
if ~exist('kerncos') | ~exist('kernsin')
   ...
   [kerncos, kernsin, freqs] = BROWN_genlgftkern(minfreq, freqrat, SR, nfreqs, windsizmax, Winnam, constwind);%,  phase0);           % GENLGFTKERN
...
[cqtrans, tt]  = BROWN_logft(infile, hopsiz, kerncos, kernsin,  windsizmax, nfreqs, SR, ccx);                                         %LOGFT
